		...
		return {
			...
			contents: [ {
				...
				},
			    {
					...
					setup: function( data ) {
						// CM-2471
						if (window.location.href.indexOf("EditTemplate") !== -1) {
							this.items.pop(); // Remove last item
							$("#" + this.domId + " select option[value='ReadMore']").each(function () {
								$(this).remove(); // Remove from select
							});
						}
