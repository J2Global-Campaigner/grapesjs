...
( function() {
	...
	function applyInlineStyle( range ) {
		...
		// CM-2400
		fixButtonHeight(range.root);        
	}

	/**
	 * Take the parent control and if it is a button, fix the font size on the A tag to match the
	 * largest font size found in the spans
	 * 
	 * @param {*} parentControl 
	 */
    function fixButtonHeight(parentControl) {
        if (parentControl.$.nodeName == 'A') {
            var parentSize = parseInt(parentControl.$.style.fontSize, 10);
            var childNodes = parentControl.$.childNodes;
            ...
            for (var c = 0; c < childNodes.length; c++) {
                ...
                var content = "<span style='font-size:" + parentSize + "px'>" + cNode.data + "</span>";

                $(cNode).replaceWith(content);
            }
	...
	/**
	 * Get the largest font size in the button (spans)
	 * 
	 * @param {*} current 
	 * @param {*} maxSize 
	 */
    function getLargestSpan(current, maxSize) {
        ...
        for (var i = 0; i < count; i++) {
            ...
            currentSize = parseInt(child.style.fontSize, 10);
            if (currentSize > maxSize) {
                maxSize = currentSize;
            }
            if (child.children.length > 0) {
                maxSize = getLargestSpan(child, maxSize);
            }
        }
